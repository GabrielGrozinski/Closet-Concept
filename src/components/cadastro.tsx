...
interface Props {
    setMostrarCadastro: (v: boolean) => void;
}
...
export default function Cadastro({setMostrarCadastro}: Props) {
    ...
    return (
        <div className="flex flex-col pb-6">
            <ArrowLeftCircle onClick={() => setMostrarCadastro(false)} className="fixed -translate-1/2 top-7 text-neutral-100 left-6 cursor-pointer" size={30} />
            ...
            :
                <button disabled={!requisitoCadastro} onClick={() => cadastro()} 
                    className={`transition-all font-medium duration-100 p-2 min-w-30 text-center rounded-2xl border border-slate-800/36 text-white
                        ${requisitoCadastro ? 'active:scale-98 cursor-pointer bg-zinc-800 ' : 'bg-zinc-800/70 opacity-70 cursor-not-allowed'}
                    `}>Cadastrar
                </button>
            }
       </div>
